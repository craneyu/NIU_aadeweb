...
Public Class astr_mail
    ...
    Sub Read_All(ByVal YY As String)
        ...
        Da = Connect2SQLDB(Acade_DB, sqlstr)
        Da.Fill(dt)
    ...
    Protected Sub But_SendMail_Click(sender As Object, e As EventArgs) Handles But_SendMail.Click
        ...
        For Each dr In dt.Rows
            ...
            daG = Connect2SQLDB(Acade_DB, SQLG)
            daG.Fill(dtG)
            Dim GP As String
            ...
            Try
                ...
                SQLStr = "insert into sendmaillist(tomail, frommail, subtxt, msg, updateYMDT) values "
                SQLStr += "('" & dr("regsno").ToString & "@ms.niu.edu.tw" & "','" & FromEmail & "','課程抵免提醒通知','" & context.ToString.Replace("'", """") & "',getdate())"
                executeSqlQuery("NIU_DBASE", SQLStr)
        ...
        ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), "msg", "alert('已完成寄出。')", True)
