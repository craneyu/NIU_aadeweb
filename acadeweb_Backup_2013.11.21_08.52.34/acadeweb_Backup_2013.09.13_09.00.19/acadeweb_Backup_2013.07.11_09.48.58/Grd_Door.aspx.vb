...
Public Class Grd_Door
    ...
    Function RD_Data(ByVal sid As String)
        ...
        For Each dr In DTA.Rows
            ...
            sql_C = "SELECT DISTINCT TOP 1 [Passed] "
            sql_C += "FROM [FitnessGrade] WHERE Regsno = N'" & dr("REGSNO").ToString & "' "
            ...
            sql_D += "WHERE(Sta.[StatusID] = Sil.[StatusID]) AND Sil.[Regsno]=N'" & dr("REGSNO").ToString & "' ORDER BY Sil.[Syear] DESC,Sil.[Seme] DESC"
    ...
    Protected Sub But_SendSingle_Click(sender As Object, e As EventArgs) Handles But_SendSingle.Click
        ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), "訊息", "alert('" & Sele_Regsno & "')", True)
    End Sub
End Class